...
import {
  usePriorities,
  priorityLabel,
} from "../../features/priorities/use-priorities";
...
export function DashboardPage() {
  ...
  const openPoolCount = sumStatuses(poolCounts, [
    TicketStatus.OPEN,
  ...
  if (loading && !summary)
    return <LoadingState>Preparing your workspace...</LoadingState>;
    ...
    <section className="page dashboard-page">
      <header className="dashboard-hero">
        <div>
          ...
          <h1>
            {greeting()}, {firstName(summary?.user?.fullName ?? user?.fullName)}
          </h1>
      ...
      {error ? (
        <div className="banner error">
          <p>{error}</p>
          <button type="button" className="btn ghost" onClick={loadSummary}>
            Try again
          </button>
        </div>
      ) : null}
      ...
      <div className="dashboard-actions">
        ...
        {canWorkTickets ? (
          <Link to="/tickets/pool" className="btn ghost">
            Open ticket pools
          </Link>
        ) : null}
        {canWorkTickets ? (
          <Link to="/tickets/handoffs" className="btn ghost">
            Review handoffs
          </Link>
        ) : null}
        {isAdmin ? (
          <Link to="/admin/management" className="btn ghost">
            Manage Nexus
          </Link>
        ) : null}
      </div>
      ...
      <section className="dashboard-section">
        <div className="dashboard-section-heading">
          ...
          <Link to="/tickets" className="dashboard-section-link">
            See all tickets
          </Link>
        </div>
        <div className="dashboard-metric-grid">
          <MetricCard
            label="My requests"
            value={sumStatuses(myTicketCounts, Object.values(TicketStatus))}
            tone="blue"
          />
          <MetricCard
            label="Awaiting assignment"
            value={myTicketCounts[TicketStatus.OPEN] ?? 0}
            tone="mint"
          />
          {canWorkTickets ? (
            <MetricCard
            ...
            <MetricCard
              label="In progress"
              value={myTicketCounts[TicketStatus.CLAIMED] ?? 0}
              tone="sky"
            />
          )}
          <MetricCard
            label={canWorkTickets ? "Tickets in my pools" : "Resolved requests"}
            value={
              canWorkTickets
                ? openPoolCount
                : (myTicketCounts[TicketStatus.CLOSED] ?? 0)
            }
            tone="peach"
          />
        </div>
      ...
      {canWorkTickets ? (
        ...
        <EmployeeWorkspace
          tickets={summary?.myTickets?.recent ?? []}
          attentionTickets={attentionTickets}
          priorities={priorities}
        />
      )}
      ...
      {isAdmin ? (
        <AdminActivity
          activity={summary?.administration?.recentActivity ?? []}
        />
      ) : null}
    </section>
  );
    ...
    <section className="dashboard-section dashboard-content-grid">
      <div className="dashboard-panel clay-card">
        ...
        <TicketPreviewList
          ...
          emptyText="Submit a ticket when you need help from an internal department."
        />
      </div>
      <AttentionPanel
  ...
  return (
    <>
      <section className="dashboard-section dashboard-content-grid">
        <div className="dashboard-panel clay-card">
          <div className="dashboard-panel-heading">
            ...
            <Link to="/tickets?view=claimed" className="dashboard-section-link">
              View all
            </Link>
          </div>
          <TicketPreviewList
            ...
            emptyText="Claim a request from one of your department pools to start working."
          />
        </div>
        <div className="dashboard-panel clay-card">
          <div className="dashboard-panel-heading">
            <div>
              ...
              <h2>Department pools</h2>
            </div>
            <Link to="/tickets/pool" className="dashboard-section-link">
              Open pools
            </Link>
          </div>
          <div className="dashboard-department-list">
            {departments.map((department) => (
              <Link
                ...
                to={`/tickets/pool?view=all&departmentId=${department.departmentId}`}
            ...
            {!departments.length ? (
              <p className="dashboard-empty-note">
                You are not mapped to an active department yet.
              </p>
            ) : null}
          </div>
        </div>
      ...
      <section className="dashboard-section dashboard-utility-grid">
        ...
        />
        <QuickStatusCard
          label="Outgoing handoffs"
          value={outgoing}
          description="Proposals still awaiting a response"
          to="/tickets/handoffs"
          tone="peach"
        />
        <AttentionPanel
          tickets={attentionTickets}
          ticketsFromPool={poolAttention}
          priorities={priorities}
        />
      </section>
    </>
  ...
  return (
    <section className="dashboard-section">
      <div className="dashboard-section-heading">
        ...
        <Link to="/admin/management" className="dashboard-section-link">
          Open management
        </Link>
      </div>
      <div className="dashboard-metric-grid dashboard-admin-metrics">
        ...
        <MetricCard
          ...
          value={users.pendingLogin ?? 0}
          tone="peach"
        />
        <MetricCard
          label="Active departments"
          value={administration?.activeDepartments ?? 0}
          tone="purple"
        />
        <MetricCard
          label="Open system tickets"
          value={tickets[TicketStatus.OPEN] ?? 0}
          tone="sky"
        />
        <MetricCard
          label="Reopened tickets"
          value={tickets[TicketStatus.REOPENED] ?? 0}
          tone="peach"
        />
      </div>
    </section>
    ...
    <div className="dashboard-panel dashboard-attention clay-card">
      <div className="dashboard-panel-heading">
        <div>
          ...
          <h2>{employee ? "Needs your attention" : "Worth a look"}</h2>
        </div>
      </div>
      {source.length ? (
        ...
        <div className="dashboard-clear-state">
          ...
          <p>
            {employee
              ...
              : "No priority items in your current queues."}
          </p>
        </div>
      )}
    </div>
...
function QuickStatusCard({ label, value, description, to, tone }) {
  return (
    <Link
      to={to}
      className={`dashboard-utility-card dashboard-utility-${tone}`}
    >
      <span>{label}</span>
      <strong>{value}</strong>
...
function DashboardTicketCard({ ticket, priorities = [], compact = false }) {
  return (
    <article
      className={`dashboard-ticket dashboard-ticket-${ticket.status?.toLowerCase() ?? "unknown"} ${compact ? "is-compact" : ""}`}
    >
      <Link
      ...
      <div className="dashboard-ticket-content">
        ...
        <div className="dashboard-ticket-people">
          <span>
            Submitted by <UserLink user={ticket.submittedBy} />
          </span>
          {ticket.agent ? (
            <span>
              Assigned to <UserLink user={ticket.agent} />
            </span>
          ) : null}
        </div>
      </div>
  ...
  return (
    <section className="dashboard-section dashboard-panel clay-card">
      <div className="dashboard-panel-heading">
        ...
        <Link to="/admin/logs" className="dashboard-section-link">
          Open logs
        </Link>
      </div>
      {activity.length ? (
        <div className="dashboard-activity-list">
          {activity.map((item, index) => (
            <div
              ...
              key={`${item.source}-${item.action}-${item.createdAt}-${index}`}
            >
              <span
                className={`dashboard-activity-dot ${item.source === "ticket" ? "is-ticket" : ""}`}
                aria-hidden="true"
              />
              <div>
                <strong>{ACTIVITY_LABELS[item.action] ?? item.action}</strong>
                <span>
                  ...
                  {item.ticket ? (
                    <>
                      {" "}
                      ·{" "}
                      <Link to={`/tickets/${item.ticket.ticketId}`}>
        ...
        <p className="dashboard-empty-note">
          System activity will appear here as Nexus is used.
        </p>
      )}
    </section>
  );
