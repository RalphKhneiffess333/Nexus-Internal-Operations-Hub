...
Use Google Gemini Flash as the initial AI provider.

Use the Strategy pattern for the AI provider integration so that Gemini can later be replaced by another provider without changing the agent orchestration logic.
...
Do not expose Gemini-specific SDK types throughout the AI module.

Gemini-specific request/response conversion must remain inside the Gemini provider implementation.
...
A tool request generated by Gemini must be treated like any other request attempting to access Nexus resources.
...
Minimize information sent to Gemini.
...
Add an AI assistant interface consistent with the existing Nexus UI.

The frontend communicates only with Nexus:

```text
Frontend → Nexus Backend → Gemini
```

Do NOT call Gemini directly from the browser.

The Gemini API key must never be exposed to frontend code.
...
Failure of Gemini must NOT affect:
...
If Gemini remains unavailable, return a controlled response to the frontend indicating that the AI assistant is temporarily unavailable.
...
* give Gemini direct database access;
* give Gemini Prisma access;
...
* expose Gemini API keys to the browser;
* allow arbitrary SQL;
...
4. Ensure Gemini cannot directly access the database.
5. Ensure all tool inputs are validated.
6. Ensure protected ticket information is authorized BEFORE being sent to Gemini.
7. Ensure no secrets or credentials are sent to Gemini.
