...
class CommandFailed extends Error {
  constructor(command, code, signal) {
    super(`${command} failed${code === null ? '' : ` with exit code ${code}`}.`);
...
function promptChoice(question, choices) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const renderMenu = () => {
      ...
      process.stdout.write(`${dim('Use ↑/↓ to move, Enter to select, Ctrl+C to cancel.')}\n`);
    };
...
async function runPrerequisitesStage() {
  ...
  info('- Node.js v20+');
  info('- npm');
  info('- PostgreSQL installed and running, or access to a database you can create/use');
  info('- A Microsoft organization / Entra tenant if you intend to test authentication');
  info('- Optional: an API client such as Postman');
  info('');
  info('Microsoft Entra authentication requires an app registration and credentials.');
  ...
  if (effect === 'skip') {
    warning('Prerequisite validation skipped. Later command failures will still be reported normally.');
    return;
  }
...
async function runDependenciesStage() {
  ...
  const backendModules = fs.existsSync(path.join(repoRoot, 'backend', 'node_modules'));
  const frontendModules = fs.existsSync(path.join(repoRoot, 'frontend', 'nexus', 'node_modules'));

  if (rootModules && backendModules && frontendModules) {
    info('Existing node_modules folders were detected for the root, backend, and frontend.');
  }
...
async function runEnvironmentStage() {
  ...
  info('The backend uses backend/.env for local app configuration and backend/.env.integration for tests.');
  info('Templates are available at backend/.env.example and backend/.env.integration.example.');
  info('The frontend can optionally use frontend/nexus/.env from frontend/nexus/.env.example.');
  info('');
  info('Backend PostgreSQL variables:');
  info('DATABASE_HOST, DATABASE_PORT, DATABASE_NAME, DATABASE_USER, DATABASE_PASSWORD, DATABASE_URL');
  info('');
  info('Microsoft Entra variables:');
  info('MICROSOFT_ENTRA_TENANT_ID, MICROSOFT_ENTRA_CLIENT_ID, MICROSOFT_ENTRA_CLIENT_SECRET,');
  info('MICROSOFT_ENTRA_REDIRECT_URI, MICROSOFT_ENTRA_SCOPES, FRONTEND_URL');
  info('');
  info('Local redirect URI: http://localhost:3000/authentication/microsoft/callback');
  ...
  info('Eurisko Academy instructors can check their Week 3 email for identity provider credentials.');
  ...
  await askDoneOrCancel('Have you finished configuring the required environment variables?');
  success('Environment configuration acknowledged');
}

async function runDatabaseStage() {
  section(4, 'Database Setup');
  info('This step will apply the Prisma migrations and seed the database with the initial identity provider, departments, and sample data.');
  info('It does not wipe existing tickets.');

  const effect = await askStageDecision('Would you like to run npm run db:setup now?');
  if (effect === 'skip') {
...
async function runUsersStage() {
  ...
  info('If you need an admin or agent, the existing interactive user CLI can add or modify users, roles, and departments.');
  info('Command: npm run seed:users');

  const effect = await askStageDecision('Would you like to add or modify application users now?');
  if (effect === 'skip') {
...
async function runSetup() {
  ...
  info('');
  info('This wizard will help you install dependencies, configure your local environment, prepare PostgreSQL, optionally seed users, run tests, and start the application.');
  info('');
  info('You can skip optional steps at any time, or cancel the setup entirely.');
...
if (require.main === module) {
  runSetup().catch((error) => {
    ...
    process.exitCode = error instanceof CommandFailed && error.code ? error.code : 1;
  });
}
